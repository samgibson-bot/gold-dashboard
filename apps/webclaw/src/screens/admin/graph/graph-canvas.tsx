  ...
  function GraphCanvas(
      ...
      function initSigma() {
        ...
        for (const edge of data.edges) {
          if (
            ...
            const sourceLabel = graph.getNodeAttribute(
              edge.source,
              'nodeLabel',
            ) as string
            const targetLabel = graph.getNodeAttribute(
              edge.target,
              'nodeLabel',
            ) as string
        ...
        graph.forEachNode(function scaleSize(nodeId) {
          ...
          graph.setNodeAttribute(
            nodeId,
            'size',
            Math.max(4, Math.min(20, 4 + degree * 1.5)),
          )
        })
    ...
    useEffect(
      function handlePendingCenter() {
        ...
        if (graph.hasNode(pendingCenter)) {
          ...
          const nodeData = graph.getNodeAttribute(
            pendingCenter,
            'nodeData',
          ) as GraphNode
          onNodeClick(nodeData)
    ...
    return (
      <div className="relative flex-1">
        <div
          ref={containerRef}
          className={cn('h-full w-full bg-primary-50 rounded-lg')}
        />
