...
async function getHostMemoryMetrics(): Promise<{
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const bytes = parseInt(value, 10) * 1024 // Convert kB to bytes
...
async function getHostCpuUsage(cores: number): Promise<number | undefined> {
  try {
    // Read /host/proc/stat for more accurate CPU usage
...
export const Route = createFileRoute('/api/admin/system')({
  server: {
    handlers: {
      GET: async () => {
        try {
          // Try to get host-level metrics first, fall back to container metrics
          ...
          // Use host metrics if available, otherwise fall back to os module
            ...
            gateway: gatewayStatus
              ? {
                  ...
                  version: gatewayStatus.version
                    ? String(gatewayStatus.version)
                    : undefined,
                  sessions: sessionsList?.sessions?.length ?? 0,
                }
              : undefined,
          }
